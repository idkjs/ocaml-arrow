...
let%expect_test _ = {
  let ts = Array.init(5, ~f=create_t);
  let table = Ppx_t.arrow_table_of_t(ts);
  let table = Arrow_c_api.Table.concatenate([table, table, table]);
  let ts = Ppx_t.arrow_t_of_table(table);
  Array.iter(ts, ~f=t =>
  ...
  {|
    ...
    ((x 9)(y 2)(z"foo 4")(x_opt())(y_opt(54.598150033144236))(z_opt()))
      |};
  let col = Arrow_c_api.Table.read(table, ~column=`Name("y"), Float);
  Stdio.printf("%s\n%!", [%sexp_of: array(float)](col) |> Sexp.to_string);
  let col = Arrow_c_api.Table.read_opt(table, ~column=`Name("y"), Float);
  Stdio.printf(
    "%s\n%!",
    [%sexp_of: array(option(float))](col) |> Sexp.to_string,
  );
  %expect
  {|
