...
let (`read(read), `write(write)) =
  F.(
    read_write_fn(
      Fields.make_creator(~x=i64, ~y=f64, ~z=str, ~z_opt=f64_opt),
    )
  );

let gen = {
  let float_gen = Float.gen_uniform_excl(-1e6, 1e6);
...
let%expect_test _ = {
  let gen = {
    let%bind.Quickcheck length = Int.gen_incl(1, 10_000);
    List.gen_with_length(length, gen);
  };

  Quickcheck.iter(
    ~trials=10,
    ~seed=`Deterministic("fortytwo"),
    gen,
    ~f=ts => {
      ...
      Exn.protect(
        ~f=
          () => {
            let hd = List.hd_exn(ts);
            Stdio.printf("z: %s\n", hd.z);
            let sum_x = List.fold(ts, ~init=0, ~f=(acc, t) => acc + t.x);
            Stdio.printf("sum_x: %d\n", sum_x);
            let sum_y = List.fold(ts, ~init=0., ~f=(acc, t) => acc +. t.y);
            Stdio.printf("sum_y: %f\n", sum_y);
            let sum_z_opt =
              List.fold(ts, ~init=0., ~f=(acc, t) =>
            ...
            let lines =
              python_read_and_rewrite(~filename, ~print_details=true);
...
let sexp_of_time_ns = time_ns =>
  Time_ns.to_string_iso8601_basic(time_ns, ~zone=Time.Zone.utc)
  |> sexp_of_string;
...
let%expect_test _ = {
  ...
  Exn.protect(
    ~f=
      () => {
        let col_v1 =
          Writer.float([|1., 2., 3., 3.14159265358979, 5.|], ~name="x");
        let col_v2 =
          Writer.float_opt(
        ...
        let col_date = {
          let d = Date.of_string("2020-01-01");
          Writer.date(
        ...
        let col_time = {
          let t = Time_ns.of_string("2021-06-05 09:36:00.123+01:00");
          Writer.time_ns([|t, t, t, t, t|], ~name="col_time");
        };

        Writer.write(filename, ~cols=[col_v1, col_v2, col_date, col_time]);
        let lines = python_read_and_rewrite(~filename, ~print_details=false);
        List.iter(lines, ~f=Stdio.printf(">> %s\n%!"));
        let table = Parquet_reader.table(filename);
        let rows = Table.num_rows(table);
        Stdio.printf("%d\n%!", rows);
        let col_v2 = Column.read_float_opt(table, ~column=`Name("col_v2"));
        let col_date = Column.read_date(table, ~column=`Name("col_date"));
        let col_time = Column.read_time_ns(table, ~column=`Name("col_time"));
        Stdio.printf(
          "%s\n%s\n%s\n%!",
          [%sexp_of: array(option(float))](col_v2) |> Sexp.to_string_mach,
          [%sexp_of: array(Date.t)](col_date) |> Sexp.to_string_mach,
          [%sexp_of: array(time_ns)](col_time) |> Sexp.to_string_mach,
        );
        ();
      },
    ~finally=() => Caml.Sys.remove(filename),
